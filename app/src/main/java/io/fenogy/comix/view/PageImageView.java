...
public class PageImageView extends AppCompatImageView {
    ...
    public void setImageDrawable(Drawable drawable) {
        ...
        //MyChanges
        ColorFilter filter = new LightingColorFilter(0xfffffff8, 0x00101000);
    ...
    private void scale() {
        ...
        if (mViewMode == Constants.PageViewMode.ASPECT_FILL) {
            ...
            //post(new ZoomAnimation(1, 1, 2.4f));
        ...
        if (w < vwidth) {
            ...
            mMaxScale = Math.max(dwidth, vwidth) * 2.0f / dwidth;
        }
        else {
            mMinScale = vwidth * 0.75f / dwidth;
            mMaxScale = Math.max(dheight, vheight) * 2.0f / dheight;
        }
        setImageMatrix(mMatrix);
    ...
    private class ZoomAnimation implements Runnable {
        public final static int ZOOM_DURATION = 600;
        float mX;
        float mY;
        float mScale;
        ...
        public void run() {
            ...
            if (t < 1f) {
                ...
                //mMatrix.preTranslate(1000.0f,800.0f);
                //mMatrix.postTranslate(-1900.0f,-110.3f);
                //mMatrix.postRotate(60.0f);
                setImageMatrix(mMatrix);
            }
        }
    }
