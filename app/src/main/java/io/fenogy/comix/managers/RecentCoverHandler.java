...
public class RecentCoverHandler extends RequestHandler {

    private final static String HANDLER_URI = "recentcover";
    private Context mContext;
    private  int mCurrentPage = 0;

    public RecentCoverHandler(Context context) {
        mContext = context;
    }
    ...
    public Result load(Request data, int networkPolicy) throws IOException {
        String path = getCoverPath(data.uri,mCurrentPage);
        BitmapFactory.Options options = new BitmapFactory.Options();
        options.inPreferredConfig = Bitmap.Config.RGB_565;
        return new Result(BitmapFactory.decodeFile(path, options), Picasso.LoadedFrom.DISK);
    }

    private String getCoverPath(Uri comicUri, int pageId) throws IOException {
        ...
        if (!coverFile.isFile()) {
            ...
            InputStream stream = parser.getPage(mCurrentPage);
            ...
            stream.close();
            stream = parser.getPage(mCurrentPage);
    ...
    public Uri getComicCoverUri(Comic comic,int pageId) {
